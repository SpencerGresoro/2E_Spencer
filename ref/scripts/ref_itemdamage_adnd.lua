...
-- UtilityManagerADND.logDebug("ref_itemdamage_adnd.lua","onInit","node",node);          
-- UtilityManagerADND.logDebug("ref_itemdamage_adnd.lua","onInit","nodeItem",nodeItem);  
...
function updateDamageString(nodeField)
  local sDamageAll = "";
--UtilityManagerADND.logDebug("ref_itemdamage_adnd.lua","updateDamageString","nodeField",nodeField);   
...
-- UtilityManagerADND.logDebug("ref_itemdamage_adnd.lua","updateDamageString","sPath",sPath);   
-- UtilityManagerADND.logDebug("ref_itemdamage_adnd.lua","updateDamageString","sModule",sModule);   
-- UtilityManagerADND.logDebug("ref_itemdamage_adnd.lua","updateDamageString","sNodePath",sNodePath);   
  if sNodePath ~= nil then
    local sFindNodePath = sNodePath;
    if (sModule ~= nil) then
      sFindNodePath = sFindNodePath .. "@" .. sModule;
    end;
--UtilityManagerADND.logDebug("ref_itemdamage_adnd.lua","updateDamageString","sFindNodePath",sFindNodePath);   
    local nodeItem = DB.findNode(sFindNodePath);
    if nodeItem then 
--UtilityManagerADND.logDebug("ref_itemdamage_adnd.lua","updateDamageString","nodeItem",nodeItem);   
      --weaponlist
...
--UtilityManagerADND.logDebug("ref_itemdamage_adnd.lua","updateDamageString","sDamageAll",sDamageAll);     
    end -- nodeItem
  end -- sPath
  return sDamageAll;
end
