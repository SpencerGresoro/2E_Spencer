...
function processImportText()
    ...
    if (sText ~= "") then
      ...
      for _,sLine in ipairs(aNPCText) do
        UtilityManagerADND.logDebug("npc_import.lua","importTextAsNPC","sLine",sLine);    
        ...
        for _, sFind in ipairs(number_matches) do
          UtilityManagerADND.logDebug("npc_import.lua","importTextAsNPC","number_matches sFind",sFind);    
          local sMatch = sFind[1];
          local sValue = sFind[2];
          if (string.match(sLine:lower(),sMatch)) then
            bProcessed = true;
...
UtilityManagerADND.logDebug("npc_import.lua","importTextAsNPC","END sParagraph",sParagraph);
            sParagraph = "";
          end
        end
