...
UtilityManagerADND.logDebug("manager_char.lua onCharItemAdd nodeItem",nodeItem,DB.getValue(nodeItem,'name'));
...
-- UtilityManagerADND.logDebug("manager_char.lua updateMoveFromEncumbrance2e nodeChar",nodeChar);
...
--UtilityManagerADND.logDebug("manager_char.lua","updateMoveFromEncumbrance1e","nodeChar",nodeChar);
  if OptionsManager.getOption("OPTIONAL_ENCUMBRANCE") == "on" then
...
-- UtilityManagerADND.logDebug("manager_char.lua","calcItemArmorClass","bNonCloakArmorWorn",bNonCloakArmorWorn);      
-- UtilityManagerADND.logDebug("manager_char.lua","calcItemArmorClass","bMagicArmorWorn",bMagicArmorWorn);      
-- UtilityManagerADND.logDebug("manager_char.lua","calcItemArmorClass","bUsingShield",bUsingShield);      
...
-- UtilityManagerADND.logDebug("manager_char.lua","calcItemArmorClass","sTypeLower",sTypeLower);      
-- UtilityManagerADND.logDebug("manager_char.lua","calcItemArmorClass","sSubtypeLower",sSubtypeLower);      
-- UtilityManagerADND.logDebug("manager_char.lua","calcItemArmorClass","nMainArmorBase",nMainArmorBase);      
-- UtilityManagerADND.logDebug("manager_char.lua","calcItemArmorClass","nMainArmorTotal",nMainArmorTotal);      
-- UtilityManagerADND.logDebug("manager_char.lua","calcItemArmorClass","bIsArmor",bIsArmor);      
-- UtilityManagerADND.logDebug("manager_char.lua","calcItemArmorClass","bIsWarding",bIsWarding);      
-- UtilityManagerADND.logDebug("manager_char.lua","calcItemArmorClass","bIsShield",bIsShield);      
-- UtilityManagerADND.logDebug("manager_char.lua","calcItemArmorClass","bIsRingOrCloak-last",bIsRingOrCloak);      
-- UtilityManagerADND.logDebug("manager_char.lua","calcItemArmorClass","----------------------------");
...
-- UtilityManagerADND.logDebug("manager_char.lua","calcItemArmorClass","nMainArmorBase2",nMainArmorBase);      
-- UtilityManagerADND.logDebug("manager_char.lua","calcItemArmorClass","nMainArmorTotal2",nMainArmorTotal);      
...
--UtilityManagerADND.logDebug("manager_char.lua","addToPowerDB","nodeItem",nodeItem);
...
--UtilityManagerADND.logDebug("manager_char.lua","addToWeaponDB","nodeWeapon",nodeWeapon);     
    end
  end
end
...
--UtilityManagerADND.logDebug("manager_char.lua","addClassProficiencyDB","nodeSource",nodeSource);
...
function addAbilityAdjustment(nodeChar, sAbility, nAdj, nAbilityMax)
    ...
    -- default to 9, that's what we default to everywhere for ability scores we dont know
      ...
      local sMsg = string.format(Interface.getString("char_abilities_message_abilityadd"), StringManager.capitalize(k), nNewScore - nCurrent, DB.getValue(nodeChar, "name", ""));
...
--UtilityManagerADND.logDebug("manager_char.lua","addProficiencyDB","nodeChar",nodeChar);
  ...
  local sMsg = string.format(Interface.getString("char_abilities_message_profadd"), DB.getValue(nodeEntry, "name", ""), DB.getValue(nodeChar, "name", ""));
...
function addSkillDB(nodeChar, sSkill, nodeSource)
  ...
  local sMsg = string.format(Interface.getString("char_abilities_message_skilladd"), DB.getValue(nodeSkill, "name", ""), DB.getValue(nodeChar, "name", ""));
...
-- UtilityManagerADND.logDebug("manager_char","addClassFeatureDB","nodeSource",nodeSource);  
  ...
  -- UtilityManagerADND.logDebug("manager_char.lua","addClassFeatureDB","sOriginalNameLower",sOriginalNameLower)
  ...
  if sOriginalNameLower:match("non%-weapon proficiency") then
    ...
    -- UtilityManagerADND.logDebug("manager_char.lua","addClassFeatureDB","sPicks-->",sPicks)  
    -- UtilityManagerADND.logDebug("manager_char.lua","addClassFeatureDB","sPickSkills-->",sPickSkills)  
...
--  outputUserMessage("char_abilities_message_featureadd", DB.getValue(vNew, "name", ""), DB.getValue(nodeChar, "name", ""));
  local sMsg = string.format(Interface.getString("char_abilities_message_featureadd"), DB.getValue(vNew, "name", ""), DB.getValue(nodeChar, "name", ""));
...
--UtilityManagerADND.logDebug("manager_char","pickWeaponProfs","rWeaponAdd",rWeaponAdd);  
...
--UtilityManagerADND.logDebug("manager_char.lua","addTraitDB","sTraitType1",sTraitType);
  ...
  elseif sTraitType == "speed" or sTraitType == "movebase" then
    ...
    if sWalkSpeed then
      ...
      local sMsg = string.format(Interface.getString("char_abilities_message_basespeedset"), nSpeed, DB.getValue(nodeChar, "name", ""));
...
--  end
  ...
  local sMsg = string.format(Interface.getString("char_abilities_message_traitadd"), DB.getValue(nodeSource, "name", ""), DB.getValue(nodeChar, "name", ""));
  outputAdvancementLog(sMsg,nodeChar,vNew.getPath());
...
function pickSkills(nodeChar, aSkills, nPicks, nProf)
  -- UtilityManagerADND.logDebug("manager_char.lua","pickSkills","aSkills",aSkills)
  -- UtilityManagerADND.logDebug("manager_char.lua","pickSkills","nPicks",nPicks)
  -- Display dialog to choose skill selection
  local rSkillAdd = { nodeChar = nodeChar, nProf = nProf };
  local wSelect = Interface.openWindow("select_dialog", "");
  -- UtilityManagerADND.logDebug("manager_char.lua","pickSkills","wSelect",wSelect)

  local sTitle = Interface.getString("char_build_title_selectskills");
  -- UtilityManagerADND.logDebug("manager_char.lua","pickSkills","sTitle",sTitle)
local sMessage = string.format(Interface.getString("char_build_message_selectskills"), nPicks);
  wSelect.requestSelection (sTitle, sMessage, aSkills, CharManager.onClassSkillSelect, rSkillAdd, nPicks);
  -- UtilityManagerADND.logDebug("manager_char.lua","pickSkills","wSelect2",wSelect)
end
...
function addLanguageDB(nodeChar, sLanguage)
  ...
  local sMsg = string.format(Interface.getString("char_abilities_message_languageadd"), DB.getValue(vNew, "name", ""), DB.getValue(nodeChar, "name", ""));
  outputAdvancementLog(sMsg,nodeChar,vNew.getPath());
...
function addBackgroundRef(nodeChar, sClass, sRecord)
  ...
  local sMsg = string.format(Interface.getString("char_abilities_message_backgroundadd"), DB.getValue(nodeSource, "name", ""), DB.getValue(nodeChar, "name", ""));
  outputAdvancementLog(sMsg,nodeChar);
...
function addRaceSelect(aSelection, aTable)
  ...
  local sMsg = string.format(Interface.getString("char_abilities_message_raceadd"), sRace, DB.getValue(nodeChar, "name", ""));
...
function addClassRef(nodeChar, sClass, sRecord)
  ...
  if (nClassWeaponPenalty > nCharWeaponPenalty or (nCharWeaponPenalty == 0)) then
    ...
    outputAdvancementLog("Weapon non-proficiency penalty set to " .. nClassWeaponPenalty,nodeChar,DB.getPath(nodeChar,"proficiencies.weapon.penalty"),nCharWeaponPenalty);
  ...
  local sMsg = string.format(Interface.getString("char_abilities_message_classadd"), DB.getValue(nodeSource, "name", ""), DB.getValue(nodeChar, "name", ""));
  outputAdvancementLog(sMsg,nodeChar);
...
function addAdvancement(nodeChar,nodeAdvance,nodeClass,nodeClassSource)
  ...
  if (nTHACO ~= 0 and nTHACO < nCurrentTHACO) or (nTHACO == 0 and nCurrentTHACO == 1) then
    ...
    outputAdvancementLog("THACO updated to new value of " .. nTHACO,nodeChar,DB.getPath(nodeChar,"combat.thaco.score"),nCurrentTHACO);
  end
  ...
  if (DataCommonADND.coreVersion == "1e") then
    for i=10,-10,-1 do
      ...
      if (not nNewTHAC) or (nNewTHAC < nCurrentTHAC) and (nNewTHAC ~= 0 or nCurrentTHAC == 1) then 
        ...
        outputAdvancementLog(string.upper(sCurrentTHAC) .. " updated from ".. nCurrentTHAC .. " to new value of " .. nNewTHAC,nodeChar,DB.getPath(nodeCharMATRIX,sCurrentTHAC),nCurrentTHAC);
  ...
  if nLevel == 1 then
    ...
    outputAdvancementLog("Initial weapon proficiency slot(s): " .. nCharWeapons,nodeChar,DB.getPath(nodeChar,"proficiencies.weapon.max"),nCharWeaponsCurrent);
    outputAdvancementLog("Initial non-weapon proficiency slot(s): " .. nCharNonWeapons,nodeChar,DB.getPath(nodeChar,"proficiencies.nonweapon.max"),nCharNonWeaponsCurrent);
  end
  ...
  if nLevel > 1 then
    ...
    UtilityManagerADND.logDebug("getActiveClassMaxLevel ",DB.getValue(oMaxClass,'name'), oMaxClass, nMaxActiveLevel)
    ...
    if (oMaxClass == nodeClass ) then
      ...
      outputAdvancementLog("Gained weapon a proficiency slot for leveling.",nodeChar,DB.getPath(nodeChar,"proficiencies.weapon.max"),nCharWeaponsCurrent);
    end
    if (nMaxActiveLevel) % nNonWeaponProfRate == 0 then
      ...
      outputAdvancementLog("Gained non-weapon a proficiency slot for leveling.",nodeChar,DB.getPath(nodeChar,"proficiencies.nonweapon.max"),nCharNonWeaponsCurrent);
    end
  end
    --UtilityManagerADND.logDebug("","manager_char.lua","nLevel-1",(nLevel-1));
    --UtilityManagerADND.logDebug("","manager_char.lua","(nLevel-1) % nWeaponProfRate",(nLevel-1) % nWeaponProfRate);
  end
...
--              elseif 
              if (nCurrentValue > nValue) then
                  -- set value
                  DB.setValue(nodeChar,sPath,"number",nValue);
                  outputAdvancementLog("Save for " .. DataCommon.saves[i] .." improved to " .. nValue,nodeChar,DB.getPath(nodeChar,sPath),nCurrentValue);
              end
  ...
  if nCurrentTurnLevel < nTurnLevel then
    ...
    outputAdvancementLog("Turning improves by " .. nTurnLevel,nodeChar,DB.getPath(nodeChar,"turn.total"),nCurrentTurnLevel);
  end
  ...
  if (nodeArcaneSlots) then
    ...
    if nNewArcaneLevel > 0 then
      ...
      outputAdvancementLog("Arcane spellcasting level is now " .. nNewArcaneLevel,nodeChar,DB.getPath(nodeChar,"arcane.totalLevel"),nPreviousArcaneLevel);
    end
    for i = 1, 9 do
      ...
      if (nSlots > 0) then
        ...
        outputAdvancementLog("Updated Arcane spell level " .. i .. " slots by " .. nSlots,nodeChar,DB.getPath(nodeChar,"powermeta.spellslots" .. i .. ".max"),nCurrentSlots);
      end
    end
  end
  -- divine
  if (nodeDivineSlots) then
    ...
    if nNewDivineLevel > 0 then
      ...
      outputAdvancementLog("Divine spellcasting level is now " .. nNewDivineLevel,nodeChar,DB.getPath(nodeChar,"divine.totalLevel"),nPreviousDivineLevel);
    end
    for i = 1, 7 do
      ...
      if (nSlots > 0) then
        ...
        outputAdvancementLog("Updated Divine spell level " .. i .. " slots by " .. nSlots,nodeChar,DB.getPath(nodeChar,"powermeta.pactmagicslots" .. i .. ".max"),nCurrentSlots);
      end
    end
  end
  ...
  if nPSPGained > 0 then
    ...
    outputAdvancementLog("Psionic power level improved to " .. nLevel,nodeChar,DB.getPath(nodeChar,"psionic.totalLevel"),nLevelPrevious);
    --ChatManager.SystemMessage("Gained [" .. nPSPGained .. "] addiontional Psionic strength points.");
    outputAdvancementLog("Gained [" .. nPSPGained .. "] addiontional Psionic strength points.",nodeChar,DB.getPath(nodeChar,"combat.psp.base"),nPSPBase);
  end
  ...
  if (nMTHACO ~= 0 and nMTHACO < nCurrentMTHACO)  or (nMTHACO == 0 and nCurrentMTHACO == 1) then
      ...
      outputAdvancementLog("MTHACO updated to new value of " .. nMTHACO,nodeChar,DB.getPath(nodeChar,"combat.mthaco.base"),nCurrentMTHACO);
  end
  -- end psionic
  ...
  if (nACNew ~= nil) then
    if (nACNew < nAC) then
      ...
      outputAdvancementLog("AC updated to new value of " .. nACNew,nodeChar,DB.getPath(nodeChar,"defenses.ac.base"),nAC);
    end
  end
  ...
  if (nSpeedNew ~= nil) then
    if (nSpeedNew > nSpeed) then
      ...
      outputAdvancementLog("Move base updated to new value of " .. nSpeedNew,nodeChar,DB.getPath(nodeChar,"speed.base"),nSpeed);
    end
  end
...
function addProficiencySlots(nodeAdvance,nodeChar)
  ...
  if (nWeaponProfs > 0) then
    ...
    outputAdvancementLog("Gained weapon proficiency slot(s): " .. nWeaponProfs,nodeChar,DB.getPath(nodeChar,"proficiencies.weapon.max"),nCharWeaponsCurrent);
  end
  ...
  if (nNonWeaponProfs > 0) then
    ...
    outputAdvancementLog("Gained non-weapon proficiency slot(s): " .. nNonWeaponProfs,nodeChar,DB.getPath(nodeChar,"proficiencies.nonweapon.max"),nCharNonWeaponsCurrent);
  end
end
...
--UtilityManagerADND.logDebug("manager_char.lua","addAttackAbilities","bItemHasWeapons",bItemHasWeapons);  
  if (bItemHasWeapons) then
    local nodeWeapons = nodeChar.createChild("weaponlist");
    if not nodeWeapons then
      return;
...
--UtilityManagerADND.logDebug("manager_char.lua","addAttackAbilities","bHasSkills",bHasSkills);  
  if (bHasSkills) then
      ...
      if (originalSkill) then
        ...
        for _, sValue in ipairs(aValues) do
          ...
          if (nCheckNew ~= nCheck) then
            ...
            outputAdvancementLog("Updated Skill: " .. sName .. " " .. sValue .. " to new value of " .. nCheckNew,nodeChar,DB.getPath(originalSkill,sValue),nCheck);
        ...
        if (statNew ~= stat) then
          ...
          outputAdvancementLog("Updated Skill:" .. sName .. " stat to new value of " .. statNew,nodeChar,DB.getPath(originalSkill,"stat"),stat);
        ...
        if (textNew ~= text ) then
          ...
          outputAdvancementLog("Updated Skill: " .. sName .. " text to new value.",nodeChar,DB.getPath(originalSkill,"text"),text);
...
--UtilityManagerADND.logDebug("manager_char.lua","addAttackAbilities","bItemHasPowers",bItemHasPowers);  
...
function addClassFeatureHelper(aSelection, rClassAdd)
  ...
  -- Add features
  for _,v in pairs(DB.getChildren(nodeSource, "features")) do
    if (DB.getValue(v, "level", 0) < 1 or DB.getValue(v, "level", 0) == rClassAdd.nLevel) and (DB.getValue(v, "specializationchoice", 0) == 0) then
...
--UtilityManagerADND.logDebug("manager_char.lua","applyEXPToActiveClasses","sClass",sClass);
...
--UtilityManagerADND.logDebug("manager_char.lua","addSkillRef","nodeSource",nodeSource);    
...
function addAdventureDB(nodeChar, sClass, sRecord)
  ...
  local sMsg = string.format(Interface.getString("char_logs_message_adventureadd"), DB.getValue(nodeSource, "name", ""), DB.getValue(nodeChar, "name", ""));
  outputAdvancementLog(sMsg,nodeChar,vNew.getPath());
...
--UtilityManagerADND.logDebug("manager_char.lua","getActiveClassMaxLevel","nodeChar",nodeChar);
  local nMaxLevel = 0;
  local oMaxClass = nil;
  for _,nodeClass in pairs(DB.getChildren(nodeChar, "classes")) do
...
UtilityManagerADND.logDebug("manager_char.lua","updateHPForLevel","nOriginalHP",nOriginalHP);    
    local nHP = DB.getValue(nodeChar,"hp.base",0);
    ...
    UtilityManagerADND.logDebug("manager_char.lua updateHPForLevel",nOriginalHP,nHP,nClassCount,nLevel,sClassName);
...
UtilityManagerADND.logDebug("manager_char.lua updateHPForLevel nHPRoll 1",nHPRoll);            
UtilityManagerADND.logDebug("manager_char.lua updateHPForLevel nConBonus 1",nConBonus);            
...
UtilityManagerADND.logDebug("manager_char.lua updateHPForLevel Dual class but new class isn't greater than previous.");
    end
...
function getConstitutionHPBonus(nodeChar,nodeClass,nodeAdvance)
  ...
  if (sConBonus ~= "" and string.match(sConBonus,"/") ) then
    ...
    if UtilityManagerADND.containsAny(DataCommonADND.fighterTypes, sClassName:lower()) or hasFeature(nodeChar,FEATURE_WARRIOR_HITPOINT_BONUS) then
      if aConBonus[2] then
...
function getClassFromName(sClass)
  local node = nil;
--UtilityManagerADND.logDebug("manager_char.lua","getClassFromName","sClass",sClass);  
...
--UtilityManagerADND.logDebug("manager_char.lua","getClassFromName","sName",sName);        
...
function getAllClassAndLevelConAdjustments(nodeChar)
  ...
  for _,nodeClass in pairs(DB.getChildren(nodeChar,"classes")) do
--UtilityManagerADND.logDebug("manager_char.lua","getAllConAdjustments","nodeClass",nodeClass);    
    local sClassName = DB.getValue(nodeClass,"name","");
    local nLevel = DB.getValue(nodeClass,"level",0);
--UtilityManagerADND.logDebug("manager_char.lua","getAllConAdjustments","sClassName",sClassName);      
--UtilityManagerADND.logDebug("manager_char.lua","getAllConAdjustments","nLevel",nLevel);      
    local nodeClass = getClassFromName(sClassName);
--UtilityManagerADND.logDebug("manager_char.lua","getAllConAdjustments","nodeClass",nodeClass);      
    if (nodeClass) then
      nFoundClassCount = nFoundClassCount + 1;
      for i=1,nLevel do
--UtilityManagerADND.logDebug("manager_char.lua","getAllConAdjustments","Level:i",i);      
        for _,nodeAdvance in pairs(DB.getChildren(nodeClass,"advancement")) do
--UtilityManagerADND.logDebug("manager_char.lua","getAllConAdjustments","nodeAdvance",nodeAdvance);
...
--UtilityManagerADND.logDebug("manager_char.lua","getAllConAdjustments","nCon",nCon);
--UtilityManagerADND.logDebug("manager_char.lua","getAllConAdjustments","nConBonus1",nConBonus);
          end
        end
      end
    end
...
--UtilityManagerADND.logDebug("manager_char.lua","getAllConAdjustments","nConBonus2",nConBonus);
  return nConBonus;
end
...
UtilityManagerADND.logDebug("manager_char.lua","reCalculateHPForMultiClass","nHPRecalculated",nHPRecalculated);                 
...
function getHPRollForAdvancement(nodeChar,nodeClass,nodeAdvance,nClassCount,nLevel)
    ...
    -- UtilityManagerADND.logDebug("manager_char.lua getHPRollForAdvancement", nHPRoll);   
    nHP = nHPRoll;
    return nHP, 0;
end
...
--UtilityManagerADND.logDebug("manager_char.lua","updateHealthScore","nodeChar",nodeChar);    
...
--UtilityManagerADND.logDebug("manager_char.lua","updateHealthScore","nConMod",nConMod);    
    if (nConMod == nil) then 
...
--UtilityManagerADND.logDebug("manager_char.lua","updateHealthScore","bNPC",bNPC);  
--UtilityManagerADND.logDebug("manager_char.lua","updateHealthScore","nodeChar",nodeChar);  
