...
function onAttackChanged(p1, p2)
  ...
  nMod = nMod + WeaponManagerADND.getToHitProfs(nodeWeapon);
...
--UtilityManagerADND.logDebug("char_Weapon.lua","onAttackAction","rActor",rActor);    
    --
...
function onDamageActionSingle(nodeDamage, draginfo)
  ...
  local _, sRecord = DB.getValue(nodeWeapon, "shortcut", "", "");
  rActor.itemPath = sRecord;
--UtilityManagerADND.logDebug("char_Weapon.lua","onDamageActionSingle","rActor",rActor);    
    --
  ...
  nDmgMod = nDmgMod + WeaponManagerADND.getToDamageProfs(nodeWeapon);
