...
function getWeaponRecord(sWeaponID)
  ...
  if sWeaponsList and sWeaponsList ~= '' then
    ...
    local rWeaponList = Utility.decodeJSON(sWeaponsList);
...
function setWeaponRecordValue(nodeCT,sWeaponID,sTag,sValue,sType)
  ...
  if sWeaponsList and sWeaponsList ~= '' then
    -- decode the json style string weaponlist entry for this npc
    local aWeaponList = Utility.decodeJSON(sWeaponsList);
    for nID,rWeaponSource in pairs(aWeaponList) do 
      if rWeaponSource.sID == sWeaponID then
        ...
        local sWeaponListChanged = Utility.encodeJSON(aWeaponList);
...
-- UtilityManagerADND.logDebug("npc_weapons_ct.lua","buildWeaponsListForCTView","aControls[i].getName()",aControls[i].getName());
...
function onAttackAction(draginfo, nodeCT, rWeapon)
  ...
  rAction.modifier = rWeapon.nAttackBonus + ActorManagerADND.getAbilityBonus(rActor, rAction.stat, "hitadj");
...
--UtilityManagerADND.logDebug("npc_weapons_ct.lua","onDamageActionSingle","nodeCT",nodeCT);    
  if not nodeCT then
    return false;
  end
  ...
  local nDmgMod = rDamage.nBonus + ActorManagerADND.getAbilityBonus(rActor, sDmgAbility, "damageadj");
