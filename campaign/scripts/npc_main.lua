...
-- this makes sure that the NPC is fully loaded if they load the NPC sheet.
-- only happens if they double click a map token for a npc that's not been
-- selected in the CT yet. --celestian
function checkLoadStatusInCT(nodeCT)
-- JPG - 2022-09-25 - Updated to support full or delayed copy
	if StringManager.startsWith(DB.getPath(nodeCT), CombatManager.CT_MAIN_PATH .. ".") then
		CombatRecordManagerADND.checkDelayedCopy(nodeCT);
	end
end
--
-- 
...
-- UtilityManagerADND.logDebug("npc_main.lua","---updateAbilityScores---","node",node);
...
-- UtilityManagerADND.logDebug("npc_main.lua","updateAbilityScores","nodeChar",nodeChar);
...
function onDrop(x, y, draginfo)
  ...
  if draginfo.isType("shortcut") then
    ...
    if not bLocked and Input.isControlPressed() and nodeSource then
      if (sClass == "npc") then
        UtilityManagerADND.logDebug("npc_main.lua","onDrop","Replacing contents of :",nodeNPC, "with :",nodeSource);
