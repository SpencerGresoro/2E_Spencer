...
function onModeChanged()
  ...
  for _,w in ipairs(getWindows()) do
    ...
    -- we dont view carried state in CT anymore
...
function addEntry(bFocus)
  ...
  if (w) then
    local node = getDatabaseNode();
--UtilityManagerADND.logDebug("char_weaponslist.lua","addEntry","node",node );  
    local nodeItem = node.getParent();
    if string.match(nodeItem.getPath(),"^item") ~= nil then
--UtilityManagerADND.logDebug("char_weaponslist.lua","addEntry","nodeItem",nodeItem);  
      local nodeWeapon = w.getDatabaseNode();
--UtilityManagerADND.logDebug("char_weaponslist.lua","addEntry","nodeWeapon",nodeWeapon);  
...
--UtilityManagerADND.logDebug("char_weaponslist.lua","onDrop","draginfo",draginfo );
  if draginfo.isType("shortcut") then
    ...
    if ( string.match(node.getPath(),"^item") or 
         ...
         string.match(node.getPath(),"^background") or 
         (string.match(node.getPath(),"^npc") and Input.isControlPressed())) then
      -- load item
...
--UtilityManagerADND.logDebug("char_weaponslist.lua","onIDChangedOnItem","sClass",sClass);
--UtilityManagerADND.logDebug("char_weaponslist.lua","onIDChangedOnItem","sRecord",sRecord);
