...
function onInit()
--UtilityManagerADND.logDebug("power_import.lua","onInit","getDatabaseNode",getDatabaseNode());
end

function createBlankPower()
    local node = DB.createChild("spell"); 
UtilityManagerADND.logDebug("power_import.lua","createTable","node",node);    
...
UtilityManagerADND.logDebug("power_import.lua","processImportText","sLine",sLine);    
...
UtilityManagerADND.logDebug("power_import.lua","processImportText","END sParagraph",sParagraph);
          sParagraph = "";
        end
      end
...
function setDisciplines(nodeSpell)
  ...
  if (sType ~= "") then
    ...
    if (sName and sDiscipline and sPsionicType) then
      ...
      UtilityManagerADND.logDebug("power_import.lua","setDisciplines","sNameOriginal",sNameOriginal); 
      UtilityManagerADND.logDebug("power_import.lua","setDisciplines","sName",sName); 
      UtilityManagerADND.logDebug("power_import.lua","setDisciplines","sDiscipline",sDiscipline); 
      UtilityManagerADND.logDebug("power_import.lua","setDisciplines","sPsionicType",sPsionicType); 
...
UtilityManagerADND.logDebug("power_import.lua","setActions","sDurationRaw",sDurationRaw);  
...
UtilityManagerADND.logDebug("power_import.lua","setActions","sDurationDice",sDurationDice); 
...
UtilityManagerADND.logDebug("power_import.lua","setActions","sDurationUnit",sDurationUnit);  
...
UtilityManagerADND.logDebug("power_import.lua","setActions","bCasterLevel",bCasterLevel);  
UtilityManagerADND.logDebug("power_import.lua","setActions","bDice",bDice);  
UtilityManagerADND.logDebug("power_import.lua","setActions","nDurationValue",nDurationValue);
UtilityManagerADND.logDebug("power_import.lua","setActions","nModValue",nModValue);
...
UtilityManagerADND.logDebug("power_import.lua","setSnWSpellLevel","sText",sText);
  local nCount = 0;
  for sCasterType, sLevel in sText:gmatch("([%w%-]+),? (%d+).. [Ll]evel") do
    local node = nodeSpell;
    nCount = nCount + 1;
UtilityManagerADND.logDebug("power_import.lua","setSnWSpellLevel","sCasterType",sCasterType);
UtilityManagerADND.logDebug("power_import.lua","setSnWSpellLevel","sLevel",sLevel);
