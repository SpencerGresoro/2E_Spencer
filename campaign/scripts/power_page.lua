...
function onInit()
  ...
  DB.addHandler(DB.getPath(node, "powers.*.memorized"), "onUpdate",  onUsesChanged);
end
...
function onClose()
  ...
  DB.removeHandler(DB.getPath(node, "powers.*.memorized"), "onUpdate",  onUsesChanged);
end
...
function updatePowerWindowUses(nodeChar, w)
  ...
  local nMemorizedTotal = DB.getValue(nodeSpell,"memorized_total",0);
  ...
  local bCanMemorize = (rGroup and rGroup.grouptype == "memorization");
    ...
    if sMode == "combat" then
      if nLevel > 0 then
        if rGroup.nPrepared > 0 and nPrepared <= 0 then
          ...
          -- this is so group headers only show when we have a spell memorized in it
          if (nMemorizedTotal <= 0) then
            bShow = false;
          end
        end
      else
   ...
   if w.header.subwindow.prepared.getValue() > 1 then
    ...
    if sMode == "preparation" then
      if nLevel == 0 then
        ...
        w.header.subwindow.usepower.setVisible(not bCanMemorize);
        w.header.subwindow.blank.setVisible(not bCanMemorize);

        w.header.subwindow.memorization_remove.setVisible(bCanMemorize);
        w.header.subwindow.memorizedcount.setVisible(bCanMemorize);
        w.header.subwindow.memorization_add.setVisible(bCanMemorize);

        w.header.subwindow.memorized_tickbox_counter.setVisible(false);
      end
    else
      --w.header.subwindow.preparedcheck.setVisible(false);
      ...
      if nLevel == 0 then
        ...
        w.header.subwindow.usepower.setVisible(false);
        -- we dont need a space here if it's got a "level"/is spell attached
        --w.header.subwindow.blank.setVisible(true);
        w.header.subwindow.blank.setVisible(false);
        
        local nPreparedValue = w.header.subwindow.memorized_tickbox_counter.getPreparedValue();
        local nMemorizedValue = w.header.subwindow.memorized_tickbox_counter.getCastValue();
        w.header.subwindow.memorized_tickbox_counter.update(sMode, false, nPreparedValue, nMemorizedValue, nPreparedValue);
        
        w.header.subwindow.memorized_tickbox_counter.setVisible(true);
        w.header.subwindow.memorization_remove.setVisible(false);
        w.header.subwindow.memorizedcount.setVisible(false);
        w.header.subwindow.memorization_add.setVisible(false);
      end
    end
  else
    w.header.subwindow.blank.setVisible(false);
    if sMode == "preparation" then
      ...
      w.header.subwindow.usepower.setVisible(false);
      w.header.subwindow.counter.setVisible(false);
      -- hide memorization if not bCaster
      w.header.subwindow.memorization_remove.setVisible(f);
      w.header.subwindow.memorizedcount.setVisible(bCanMemorize);
      w.header.subwindow.memorization_add.setVisible(bCanMemorize);
      w.header.subwindow.memorized_tickbox_counter.setVisible(bCanMemorize);
      ---
...
--UtilityManagerADND.logDebug("power_page.lua","onDrop","draginfo",draginfo);  
