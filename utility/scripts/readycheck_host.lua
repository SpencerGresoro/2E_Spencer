...
--UtilityManagerADND.logDebug("readycheck_host.lua","updateName","node",node);
--UtilityManagerADND.logDebug("readycheck_host.lua","updateName","sName",sName);
...
--UtilityManagerADND.logDebug("readycheck_host.lua","updateName","sID",sID);
...
function seeIfEveryoneIsReady()
  ...
  for _,node in pairs(DB.getChildren("connectedlist")) do
    ...
    if (not bReady) then
      UtilityManagerADND.logDebug("ready_check_host.lua","seeIfEveryoneIsReady",sName .. " is NOT READY");
      bEveryoneReady = false;
    end
  end
  -- DING dm that everyone is ready?
  if bEveryoneReady then
    UtilityManagerADND.logDebug("ready_check_host.lua","EVERYONE READY!");
