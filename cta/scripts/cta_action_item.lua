...
function refreshText()
    ...
    local sText = "";
    --UtilityManagerADND.logDebug("cta_action_item.lua","refreshText","nodeEntry",nodeEntry);            
    ...
    --UtilityManagerADND.logDebug("cta_action_item.lua","refreshText","sText",sText);           
    setValue(sText);
end
--[[
    Add a power entry
...
function addHyperTextRecord(sText,rRecord,nodeEntry,bNoBrackets,bNoHighlight)
    ...
    if rRecord.name:len() < 1 then
        UtilityManagerADND.logDebug("cta_action_item.lua","addHyperTextRecord","Name empty: rRecord",rRecord); 
    end
...
--UtilityManagerADND.logDebug("cta_action_item.lua","onHover","oncontrol",oncontrol);                
    if dragging then
        return;
    end
...
function onClickDown(button, x, y)
    --UtilityManagerADND.logDebug("cta_action_item.lua","onClickDown","button",button);       
...
function onDoubleClick(x, y)
    --UtilityManagerADND.logDebug("cta_action_item.lua","onDoubleClick","x",x);          
...
--UtilityManagerADND.logDebug("cta_action_item.lua","onDragStart","draginfo",draginfo);        
    dragging = false;
...
-- UtilityManagerADND.logDebug("cta_action_item.lua","actionAbility","rRecord",rRecord);       
-- UtilityManagerADND.logDebug("cta_action_item.lua","actionAbility","rRecord.type",rRecord.type);            
	local bResult = true;
    if not rRecord then
        return false;
    end
...
function sortByOrders(aTable)
    local function sortByOrder(a,b)
        return DB.getValue(a,"order",1) < DB.getValue(b,"order",1);
    end
    local aSorted = {};
