...
--[[
	This was originally copied over from CoreRPG. We had to override for the many differences in allowing NPCs to have items and
	various other AD&D ruleset changes.

	I've last compared it to v2021-11-15 of CoreRPG for fixes they made but not sure I need to anymore... --celestian
]]
...
local aDeleteCopyFields = { "count", "locked", "location", "carried", "showonminisheet", "assign", "weighttotal" };
...
-- Not sure why they made this, for now it's just going to sit here and we'll
-- continue to use the old method above
function getItemSourceTypeCORERPG_CHANGE(vNode)
	local sNodePath = nil;
...
--UtilityManagerADND.logDebug("manager_item.lua","handleItem","sTargetRecordType",sTargetRecordType );
...
function addItemToList(vList, sClass, vSource, bTransferAll, nTransferCount)
    ...
    -- this makes sure ID states are correct when added --celestian
    ...
    ---
		bCopy = true;
    -- see if item has subitems -- celestian
    if ItemManager2 and ItemManager2.addItemToList2 and DB.getChildCount(nodeSource, "subitems") > 0 then
      bCopy = ItemManager2.addItemToList2(sClass, nodeSource, nodeTemp, nodeList);
    end
  -- leaving this bit so I dont break previous behavior
    ...
    if
       (
        sSourceRecordType == "treasureparcels" and (sTargetRecordType == "partysheet" or sTargetRecordType == "charsheet" or sTargetRecordType == "npc")
       ) or
       (
        sSourceRecordType == "npc" and (sTargetRecordType == "partysheet" or sTargetRecordType == "treasureparcels")
       ) or 
       (
        sSourceRecordType == "partysheet" and sTargetRecordType == "treasureparcels"
       ) or
       (
        sSourceRecordType == "treasureparcels" and sTargetRecordType == "treasureparcels"
       ) then
			bCountN = true;
		elseif (sSourceRecordType == "partysheet" and sTargetRecordType == "charsheet") or
				(
         sSourceRecordType == "charsheet" and (sTargetRecordType == "charsheet" or sTargetRecordType == "partysheet")
        ) then
			if bTransferAll then
				bCountN = true;
			end
		elseif (sSourceRecordType == "temp" and sTargetRecordType == "charsheet") or
				(
         sSourceRecordType == "temp" and (sTargetRecordType == "treasureparcels" or sTargetRecordType == "partysheet") 
        ) then
			bCountN = true;
		end
		local nCount = 1;
