...
function getRoll(rActor,nTargetDC, bSecretRoll)
  ...
  local _, nodeChar = ActorManager.getTypeAndNode(rActor);
...
function modRoll(rSource, rTarget, rRoll)
  ...
  if rSource then
    ...
    --[[
      Can't have turn resist modifiers on npcs, pc needs to have the effect applied. You don't turn per npc, you turn
      them all at once.... which means it doesn't check each individual target for effects that might adjust the source's 
      turn value --celestian
    ]]--
    
    -- -- apply turn roll modifiers from the target
    -- aAddDice, nAddMod, nEffectCount = EffectManager5E.getEffectsBonus(rTarget, {"TURNRESIST"}, false);
-- UtilityManagerADND.logDebug("manager_action_turnundead.lua","modRoll","nAddMod",nAddMod);    
    -- if (nEffectCount > 0) then
      -- bEffects = true;
    -- end
    -- rRoll.nMod = rRoll.nMod + nAddMod;
    ...
    -- -- apply turn level adjustment from the target
    -- aAddDice, nAddMod, nEffectCount = EffectManager5E.getEffectsBonus(rTarget, {"TURNLEVELRESIST"}, false);
-- UtilityManagerADND.logDebug("manager_action_turnundead.lua","modRoll","nAddMod2",nAddMod);    
    -- if (nEffectCount > 0) then
      -- bEffects = true;
    -- end
    -- rRoll.nTarget = rRoll.nTarget + nAddMod;
...
function onRoll(rSource, rTarget, rRoll)
  ...
  if rRoll.nTarget then
    ...
    if (bTurnedSome) then
      ...
      UtilityManagerADND.logDebug("manager_action_turnundead.lua","onRoll","sTurn=",sTurn);
    ...
    if (bTurnedSome) then
      ...
      if (nodeTargets ~= nil) then
        ...
        if (#aTargets > 0) then 
          ...
          UtilityManagerADND.logDebug("manager_action_turnundead.lua","onRoll","Turned Slots, 2d6, nTurnBase=",nTurnBase);
          ...
          for i=1, #aHDTurn do
            ...
            UtilityManagerADND.logDebug("manager_action_turnundead.lua","onRoll","Turned For Max HD=",aHDTurn[i].nHD);
            if bDestroyPlus then
            -- destroy+ turn (add 2d4)
              nTurnExtra = StringManager.evalDice(aExtraTurn, 0);
              UtilityManagerADND.logDebug("manager_action_turnundead.lua","onRoll","Gained extra turn, 2d4, nTurnExtra=",nTurnExtra," For HD=",aHDTurn[i].nHD);
            ...
            for nID=1,#aTargets do
              ...
              if (sNodeName ~= nil) then
                ...
                local nNPCHD = CombatManagerADND.getNPCLevelFromHitDice(DB.getValue(node,"hitDice","1"),node);
                UtilityManagerADND.logDebug("manager_action_turnundead.lua","onRoll","Checking [" .. node.getName() .. "] nNPCHD=",nNPCHD);
                ...
                if (sCreatureType:match("undead") ~= nil and nNPCHD <= aHDTurn[i].nHD and not getAlreadyTurned(aTurnedList,node)) then
...
function handleTurn(rSource, nodeTurn,rMessage,bDestroy)
  local rTarget = ActorManager.resolveActor(nodeTurn);
...
function applyTurnedState(rSource, rTarget)
  ...
  if not EffectManager5E.hasEffect(rTarget, "Turned") then
    EffectManager.addEffect("", "", ActorManager.getCTNode(rTarget), { sName = "Turned", nDuration = 0, sSource = ActorManager.getCTNodeName(rSource), nGMOnly = 0 }, true);
  end
end
