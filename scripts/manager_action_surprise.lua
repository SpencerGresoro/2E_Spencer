...
function performRoll(draginfo, rActor, nTargetDC, bSecretRoll)
  local rRoll = getRoll(rActor, nTargetDC, bSecretRoll);
  ...
  local nAddMod = 0;
    ...
    if rSource then
      ...
      -- Get ability modifiers
       local nAbilityAdj = ActorManagerADND.getAbilityBonus(rSource, "dexterity", "reactionadj");
       if nAbilityAdj ~= 0 then
        rRoll.sDesc = rRoll.sDesc .. " [DEX" .. UtilityManagerADND.getNumberSign(nAbilityAdj) .. "]";
        nAddMod = nAddMod + nAbilityAdj;
       end
